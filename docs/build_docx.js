...
const children = [
  ...
  DEF('Forward-head / cranio-vertebral offset (new)',
  ...
  DEF('Head pitch / chin position (new)',
  ...
  BULLET('Angle basis. The head-support angle is computed in 3-D (depth-aware) when the world landmarks are available — more robust to perspective than a flat photo — but the depth itself is estimated from a single camera, so it is better than 2-D, not lab goniometry. The forward-head and chin measures remain apparent (2-D image-plane).'),
  BULLET('2-D cannot fully separate rotation from lateral tilt. Preferring the visible ear and using 3-D for the headline angle reduce this, but do not eliminate it.'),
  BULLET('It depends on what the camera can see. Blankets, pillows, and clothing can occlude landmarks. When the app cannot see enough to measure something, it says “could not assess” rather than producing a number. The fuller skeleton (hands, feet, face) is low-confidence at 6–8 ft and only drawn when actually detected; no measurement depends on those distal points.'),
  BULLET('Derived reference points are estimates, not detected anatomy. The trunk reference dots are a straight-line interpolation between the shoulder and hip midpoints and CANNOT show true spinal curvature (no model detects spine points between the shoulders and hips). The neck and head-top markers are likewise estimates. These drive no findings — they are visual references only.'),
  BULLET('The thresholds are heuristics, not clinical cutoffs. The head-support bands are < 8° = near neutral, 8–18° = mild, > 18° = priority; the new forward-head and chin bands are likewise starting points. All are explicitly intended to be calibrated by you.'),
