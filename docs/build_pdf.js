...
bullets([
  'On-device only. Everything runs in the phone’s web browser. The video feed is processed live on the device using Google’s MediaPipe Pose Landmarker — a body-keypoint model that estimates 33 anatomical landmarks per frame.',
  ...
  'Outputs. An annotated image (body landmarks, a spine reference line, the measured angle, and numbered markers on flagged areas) plus a plain-text report. The patient can capture all three views as a set and send them together.'
]);
...
bullets([
  'Angles are “apparent,” not true goniometry. They are measured in the 2-D image plane from a single photo. Camera height, tilt, and perspective all affect them; they indicate direction and trend more reliably than exact degrees.',
  '2-D cannot fully separate rotation from lateral tilt. Preferring the visible ear reduces this, but does not eliminate it.',
  'It depends on what the camera can see. Blankets, pillows, and clothing can occlude landmarks. When the app cannot see enough to measure something, it says “could not assess” rather than producing a number.',
  'The thresholds are heuristics, not clinical cutoffs. The head-support bands currently used are < 8° = near neutral, 8–18° = mild, and > 18° = priority. These were chosen as reasonable starting points and are explicitly intended to be calibrated by you.',
